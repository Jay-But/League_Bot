...
class DraftCog(commands.Cog):
    ...
    async def startdraft(self, interaction: discord.Interaction, rounds: int, picks_per_round: int):
        ...
        if rounds < 1 or picks_per_round < 1:
            ...
            return
        if picks_per_round > len(guild_config.get("teams", [])):
    ...
    async def enddraft(self, interaction: discord.Interaction):
        ...
        async def enddraft_callback(interaction: discord.Interaction):
            ...
            await self.log_action(interaction, "Draft Ended", "Draft terminated")
    ...
    async def setpick(self, interaction: discord.Interaction, player: discord.Member, team: str):
        ...
        team_role, team_name, _ = self.get_team_info(guild_config, interaction.user)
        if team_name != team:
        ...
        player_team_role, player_team, _ = self.get_team_info(guild_config, player)
        if player_team:
        ...
        current_roster = len(self.get_team_members(interaction.guild, team)) # get_team_members doesn't need guild_config
    ...
    async def autopick(self, interaction: discord.Interaction, team: str):
        ...
        await self.log_action(interaction, "Auto-pick Toggled", f"{team} set autopick to {status}")
